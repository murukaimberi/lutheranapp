...
type RestOf<T extends IPost | NewPost> = Omit<T, 'postedDate'> & {
...
export class PostService {
  ...
  protected convertDateFromClient<T extends IPost | NewPost | PartialUpdatePost>(post: T): RestOf<T> {
    return {
      ...post,
      postedDate: post.postedDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restPost: RestPost): IPost {
    return {
      ...restPost,
      postedDate: restPost.postedDate ? dayjs(restPost.postedDate) : undefined,
    };
  }
